...
<AppHead title="Command Center - Radiologi" />

<div class="min-h-screen bg-slate-100 flex flex-col font-sans text-slate-900">
    <DashboardHeader 
        title="Radiologi" 
        subtitle="Radiologi"
        iconPath="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607zM10.5 7.5v6m3-3h-6"
    />
    ...
    <main class="flex-1 max-w-[1600px] w-full mx-auto px-4 sm:px-8 py-4 sm:py-8 flex flex-col gap-6 sm:gap-8">
        ...
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {#each [
                { label: 'Total Pemeriksaan Hari Ini', value: manager.totalVisitsToday,       delta: manager.visitsDelta, sub: 'Selesai hari ini' },
                { label: 'Belum Selesai Ekspertise',value: manager.totalQueueCount,        delta: manager.queueDelta, sub: 'Dalam proses' },
                { label: 'Dokter Radiologi Aktif',          value: database.activeDoctorsCount,   delta: null, sub: 'Bertugas hari ini' },
            ] as kpi, ki (ki)}
            ...
            <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100 flex flex-col gap-3">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Rata-rata Waktu Pemeriksaan</p>
                {#if manager.isLoading}
                ...
                {/if}
                <div class="flex items-center justify-between mt-auto pt-3 border-t border-slate-50">
                    <p class="text-[10px] text-slate-400 font-medium">Dari registrasi ke pemeriksaan</p>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
            ...
            <div class="xl:col-span-7 bg-white rounded-xl shadow-sm border border-slate-100 p-6 animate-fade-in" style="animation-delay: 300ms">
                <div class="flex flex-col sm:flex-row justify-between items-start mb-5 gap-3">
                    <div>
                        <h2 class="text-base font-black text-slate-900">Tren Layanan Radiologi</h2>
                        <p class="text-xs text-slate-400 font-medium mt-0.5 text-ellipsis">Volume pemeriksaan bulanan tahun {manager.selectedYear}</p>
                    </div>
                ...
                <div class="mt-8 pt-6 border-t border-slate-50 grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="flex flex-col">
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-tight">Total Pemeriksaan</span>
                    ...
                    <div class="flex flex-col border-l border-slate-100 pl-4">
                        ...
                        <div class="flex items-baseline gap-1.5 mt-1">
                            <span class="text-2xl font-black text-primary leading-none">{trendSummary.peak}</span>
                            <span class="text-[10px] font-bold text-primary/70">{manager.trendPoints[peakIdx]?.label || '-'}</span>
                        </div>
                    </div>
                    <div class="flex flex-col border-l border-slate-100 pl-4">
            ...
            <div class="xl:col-span-5 flex flex-col gap-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex-1">
                    <h2 class="text-base font-black text-slate-900 mb-1">Breakdown Asuransi</h2>
                    <p class="text-xs text-slate-400 font-medium mb-6">Penjamin pasien radiologi tahun {manager.selectedYear}</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                <div>
                    <h2 class="text-base font-black text-slate-900 tracking-tight">Statistik Tindakan Radiologi</h2>
                    <p class="text-xs text-slate-400 font-medium mt-0.5">
                        Agregat tindakan radiologi hari ini dan tren mingguan.
                    </p>
                </div>
                <span class="text-[10px] font-black px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-lg text-slate-500 uppercase tracking-wider">
                    {manager.clinics.length} Jenis Tindakan
                </span>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <th class="px-6 py-3 text-left">Nama Tindakan</th>
                            <th class="px-4 py-3 text-center">Kategori</th>
                            <th class="px-4 py-3 text-right">Volume Hari Ini</th>
                            <th class="px-4 py-3 text-right">Rata-rata Waktu (mnt)</th>
                            <th class="px-4 py-3 text-center">Tren Layanan (7H)</th>
                            <th class="px-6 py-3 text-right">vs Minggu Lalu</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        {#each manager.clinics as radExam (radExam.id)}
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                <td class="px-6 py-4 font-black text-slate-800">{radExam.doctor}</td>
                                ...
                                <td class="px-4 py-4 text-right">
                                    <p class="font-black text-slate-900 tabular-nums">{radExam.totalVisitsToday}</p>
                                    <p class="text-[10px] text-slate-400">pemeriksaan</p>
                                </td>
                                <td class="px-4 py-4 text-right font-black tabular-nums" style="color: {waitColor(radExam.avgWaitTime)}">
                                    {radExam.avgWaitTime}
                                </td>
                                <td class="px-4 py-4">
                                    <div class="flex justify-center">
                                        <Sparkline
                                            data={radExam.weeklyTrend}
                                            color={radExam.vsLastWeek >= 0 ? '#0093dd' : '#ef4444'}
                                        />
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <span class="text-xs font-black px-2 py-0.5 rounded {deltaClass(radExam.vsLastWeek)}">
                                        {deltaIcon(radExam.vsLastWeek)} {Math.abs(radExam.vsLastWeek)}%
                                    </span>
                                </td>
                            </tr>
                        {/each}
                        ...
                        {#if manager.clinics.length === 0}
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-slate-400 font-medium">Belum ada pemeriksaan radiologi hari ini.</td>
                            </tr>
                        {/if}
                    </tbody>
                </table>
