<script lang="ts">
    ...
    const statusLabels: Record<string, string> = {
        active: 'Aktif', break: 'Istirahat', closed: 'Tidak Aktif',
    };
...
<AppHead title="Command Center - IGD" />
...
<div class="min-h-screen bg-slate-100 flex flex-col font-sans">
    ...
    <main class="flex-1 max-w-[1600px] w-full mx-auto px-4 sm:px-8 py-4 sm:py-8 flex flex-col gap-6 sm:gap-8">
        ...
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {#each [
                { label: 'Total Kunjungan IGD', value: manager.totalVisitsToday,       delta: manager.visitsDelta, sub: 'Hari ini' },
                { label: 'Pasien Belum Ditangani',value: manager.totalQueueCount,        delta: manager.queueDelta, sub: 'Masih antre' },
                { label: 'Dokter Jaga',          value: database.activeDoctorsCount,   delta: null, sub: 'Dokter aktif hari ini' },
            ] as kpi, ki (ki)}
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
            ...
            <!-- Trend Chart -->
            <div class="xl:col-span-7 bg-white rounded-xl shadow-sm border border-slate-100 p-6 animate-fade-in" style="animation-delay: 400ms">
                <div class="flex flex-col sm:flex-row justify-between items-start mb-5 gap-3">
                    <div>
                        <h2 class="text-base font-black text-slate-900">Tren Kunjungan IGD</h2>
            ...
            <div class="xl:col-span-5 flex flex-col gap-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex-1">
                    <h2 class="text-base font-black text-slate-900 mb-1">Populasi Asuransi di IGD</h2>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                <div>
                    <h2 class="text-base font-black text-slate-900 tracking-tight">Performa Dokter Jaga IGD</h2>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            ...
                            <th class="px-4 py-3 text-right">Sedang Ditangani</th>
                            <th class="px-4 py-3 text-right">Rata-rata Penanganan (mnt)</th>
                            <th class="px-4 py-3 text-center">Tren Layanan</th>
                            <th class="px-6 py-3 text-right">vs Minggu Lalu</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        {#each manager.clinics as doctor (doctor.id)}
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                <td class="px-6 py-4 font-black text-slate-800">{doctor.doctor}</td>
                                <td class="px-4 py-4 text-center">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase
                                        {doctor.status === 'active' ? 'bg-emerald-50 text-emerald-700' : doctor.status === 'break' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-500'}">
                                        <span class="w-1.5 h-1.5 rounded-full {statusDot[doctor.status]}"></span>
                                        {statusLabels[doctor.status]}
                                    </span>
                                </td>
                                <td class="px-4 py-4 text-right">
                                    <p class="font-black text-slate-900 tabular-nums">{doctor.totalVisitsToday}</p>
                                    <p class="text-[10px] text-slate-400">pasien</p>
                                </td>
                                <td class="px-4 py-4 text-right font-black text-slate-900 tabular-nums">{doctor.queueCount}</td>
                                <td class="px-4 py-4 text-right font-black tabular-nums" style="color: {waitColor(doctor.avgWaitTime)}">
                                    {doctor.avgWaitTime}
                                </td>
                                <td class="px-4 py-4">
                                    <div class="flex justify-center">
                                        <Sparkline
                                            data={doctor.weeklyTrend}
                                            color={doctor.vsLastWeek >= 0 ? '#0093dd' : '#ef4444'}
                                        />
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <span class="text-xs font-black px-2 py-0.5 rounded {deltaClass(doctor.vsLastWeek)}">
                                        {deltaIcon(doctor.vsLastWeek)} {Math.abs(doctor.vsLastWeek)}%
                                    </span>
                                </td>
                            </tr>
                        {/each}
                        ...
                        {#if manager.clinics.length === 0}
                            <tr>
                                <td colspan="7" class="px-6 py-8 text-center text-slate-400 font-medium">Belum ada kunjungan dokter hari ini.</td>
                            </tr>
                        {/if}
                    </tbody>
                </table>
